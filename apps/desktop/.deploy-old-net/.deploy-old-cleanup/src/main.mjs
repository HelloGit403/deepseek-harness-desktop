...
import { probeHarness, waitForHarness } from './harness-server.mjs'
...
function startHarnessServer() {
  ...
  serverProcess = spawn(process.execPath, ['--expose-internals', resolveDshEntry(), 'web', '--port', APP_PORT], {
...
if (!app.requestSingleInstanceLock()) {
  ...
  app.whenReady().then(openHarness).catch((error) => {
    mainWindow ??= createMainWindow()
