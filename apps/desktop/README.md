...
The Desktop Upstream Sync workflow checks the configured official branch every three hours. When its head changes, the workflow records the new commit, advances the desktop release-candidate version, and dispatches Desktop Release. If the matching release is absent after a failed or interrupted run, the next sync dispatches it again without another version bump. A release appears in the application channel only after the complete official build, desktop tests, installer build, and packaged startup smoke pass.
