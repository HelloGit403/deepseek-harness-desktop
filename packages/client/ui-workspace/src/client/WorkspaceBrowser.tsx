...
/** Archived Sessions stay discoverable here and can be restored without opening a dialog. */
function ArchivedSessions({
  ...
  const [pendingIds, setPendingIds] = useState<readonly SessionId[]>([])
  const [restoreError, setRestoreError] = useState<string | null>(null)
  const workspaceBySession = useMemo(() => {
    const result = new Map<SessionId, string>()
    for (const workspace of workspaces) {
      for (const sessionId of workspace.sessionIds) result.set(sessionId, workspace.title)
    }
    return result
  }, [workspaces])
  const items = useMemo(() => {
    ...
    return archivedSessionIds
      ...
      .filter((session) => {
        ...
        const workspaceTitle = workspaceBySession.get(session.id) ?? t('group.ungrouped')
        return title.toLocaleLowerCase().includes(normalized)
          || workspaceTitle.toLocaleLowerCase().includes(normalized)
      })
      .sort((a, b) => b.updatedAt - a.updatedAt || a.id.localeCompare(b.id))
  }, [archivedSessionIds, list.byId, query, t, workspaceBySession])
  const restore = (sessionId: SessionId): void => {
    setPendingIds(current => current.includes(sessionId) ? current : [...current, sessionId])
    setRestoreError(null)
    unarchiveSession(sessionId).catch(() => {
      setRestoreError(t('archive.restoreFailed'))
    }).finally(() => {
  ...
  return (
    <div className={clsx(css.treeBody, css.wide)}>
      <div className={css.archiveIntro}>
        <span className={css.archiveIntroIcon}><IconArchiveOutline20 size={18} /></span>
        <span>
          <strong>{t('archive.title')}</strong>
          <small>{t('archive.description')}</small>
        </span>
      </div>
      {restoreError !== null && <div className={css.archiveError} role="alert">{restoreError}</div>}
      <div className={clsx(css.list, css.archiveList)} role="list" aria-label={t('archive.list.aria')}>
        {items.map((session) => {
          const pending = pendingIds.includes(session.id)
          const title = session.blank ? t('session.new') : session.displayTitle
          const workspaceTitle = workspaceBySession.get(session.id) ?? t('group.ungrouped')
          const relative = relativeTime(session.updatedAt, Date.now())
          const updated = relative.unit === 'now'
            ? t('time.now')
            : t('time.ago', { t: t(`time.${relative.unit}`, { n: relative.n }) })
          return (
            <div className={css.archiveRow} role="listitem" key={session.id}>
              <button
                type="button"
                className={css.archiveSession}
                aria-label={t('archive.openSession', { name: title })}
                onClick={() => { open(session.id) }}
              >
                <span className={css.archiveSessionIcon}><IconArchiveOutline20 size={16} /></span>
                <span className={css.archiveSessionText}>
                  <span className={css.archiveSessionTitle}>{title}</span>
                  <span className={css.archiveSessionMeta}>{workspaceTitle} · {updated}</span>
                </span>
              </button>
              <button
                type="button"
        ...
        {items.length === 0 && (
          <div className={css.empty}>
            {query === '' ? t('archive.empty') : t('archive.noMatches')}
          </div>
        )}
      </div>
      <span className={css.fade} />
...
export function WorkspaceBrowser({
  ...
  useEffect(() => {
    if (normalizedQuery === '' || showArchived) {
  ...
  }, [normalizedQuery, searchSessions, showArchived])
      ...
      <div className={css.sectionHeader}>
        {wide && (
          <span className={clsx(css.sectionLabel, css.wide, searchExpanded && css.sectionLabelHidden)}>
            {showArchived ? t('section.archived') : groupBy === 'flat' ? t('section.sessions') : t('section.workspaces')}
          </span>
        )}
        {wide && (
        ...
        <div className={clsx(css.headerActions, wide && searchExpanded && css.headerActionsHidden)}>
          ...
          {wide && (
            <Tooltip label={showArchived ? t('archive.hide') : t('archive.open')} side="bottom" delayMs={500}>
              <button
                type="button"
                className={clsx(css.iconButton, css.archiveButton, showArchived && css.iconButtonActive)}
                aria-label={t('archive.count.aria', { n: archivedSessionIds.length })}
              ...
              >
                ...
                {archivedSessionIds.length > 0 && (
                  <span className={css.archiveCount} aria-hidden="true">
                    {archivedSessionIds.length > 99 ? '99+' : archivedSessionIds.length}
                  </span>
                )}
              </button>
            </Tooltip>
