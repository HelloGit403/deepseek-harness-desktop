...
The repository's Windows release workflow publishes the NSIS installer, its blockmap, and `latest.yml` as one GitHub Release. The workflow derives the release tag from the desktop package version and rejects a mismatched pushed tag or an incomplete asset set.
