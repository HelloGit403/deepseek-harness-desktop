...
The repository's Windows release workflow checks out the exact official DeepSeek Harness revision recorded in `.github/desktop-upstream.json` into an isolated source directory, then replaces that temporary tree's desktop package with the owned desktop shell and copies the desktop-only Settings integration onto the coherent official tree. It preserves the official workspace configuration, injects only the reviewed Electron dependency build permissions, and removes the replaced upstream desktop project from the temporary Host compiler and bundler inputs, so new upstream install requirements remain authoritative without compiling two desktop implementations. It installs that assembled workspace, builds the official Host and Web artifacts, and publishes the NSIS installer, its blockmap, and `latest.yml` as one GitHub Release. The staged application includes the build Node.js executable and its complete license; the Harness process therefore uses the same native-module ABI that installed its packages instead of Electron's embedded Node.js ABI. Installer packaging removes partial output and retries twice after an initial failure because electron-builder downloads platform tool archives while it runs. The workflow derives a `v<version>` tag from the desktop package version because `electron-updater` requires the release-feed tag to parse as SemVer, and rejects a mismatched pushed tag, a repository build failure, a packaged Harness startup failure, or an incomplete asset set. The scheduled sync queries the matching GitHub Release through an authenticated API request: HTTP 404 dispatches the missing release, HTTP 200 leaves the current channel unchanged, and any other response stops the sync.
