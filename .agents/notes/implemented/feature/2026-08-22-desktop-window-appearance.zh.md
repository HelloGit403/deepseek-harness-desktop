...
Electron 主进程在创建窗口前选择深色原生主题。Electron 隐藏默认标题栏内容，通过 36 像素高的标题栏覆盖层保留 Windows 原生标题栏按钮，并使覆盖层颜色与桌面客户端的深色外壳一致。renderer 预留同样的 36 像素，除原生控件占用的最右侧 138 像素外，均作为拖动区。窗口缩放、标题栏按钮行为、键盘访问、最大化窗口几何以及跨显示器移动仍由 Windows 持有。
...
**用透明无边框窗口替代全部原生窗口栏。** 完全自定义的边框需要自行持有缩放行为、标题栏按钮、键盘访问、最大化窗口几何与显示缩放测试。隐藏标题栏覆盖层保留了这些原生行为，只需在 renderer 中为原生控件之外的区域提供拖动条。
...
安装后的 Windows 应用以深色原生窗口控件打开，可以通过拖动未被遮挡的顶部区域跨屏移动，并提供对包括原生窗口栏在内的整个窗口生效、可持久保存的 60%–100% 透明度控制。拖动条不能包含可交互的客户端控件；其固定高度与右侧排除范围必须和 Electron 标题栏覆盖层保持一致。降低透明度会有意降低文本对比度，但强制下限和一键恢复 100% 使用户始终能够恢复。读取偏好失败时窗口回退到完全不透明，无效 renderer 值会被拒绝，保存失败则会在设置行中明确显示。浏览器部署和 Agent 行为不变。
