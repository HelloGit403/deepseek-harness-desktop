...
仓库中的 Windows 发布工作流会把 NSIS 安装包、对应的 blockmap 和 `latest.yml` 作为一个 GitHub Release 发布。工作流从桌面包版本推导发布标签，并拒绝不匹配的已推送标签或不完整的发布文件集合。
...
每个可安装更新都必须在桌面适配通过检查后，把 `latest.yml`、NSIS 安装包及其 blockmap 放在同一个 Release 中发布。应用能够发现并安装这些版本，而无需移动用户数据。公开通道依赖仓库 Release 可用；开发构建和未打包构建不会发起更新网络请求。
